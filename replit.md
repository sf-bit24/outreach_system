...
1. **Data Acquisition** — three paths:
   ...
   - **Apollo + LinkedIn scrapers** (browser-session) via `POST /api/sources/scraper/{credentials,jobs}` using Playwright + system Chromium. Cookies are AES-256-GCM-encrypted with a key derived from `SESSION_SECRET` and stored in `scraping_credentials`. Jobs run in the background (`scraping_jobs` table) with rate limits (100/h LinkedIn, 200/h Apollo, 150/h Google Maps) and 8-25s jitter delays. Set `SCRAPING_DRY_RUN=1` to bypass real browser calls during local testing.
  - **Google Maps scraper** (no login required, public business listings) via the same `POST /api/sources/scraper/jobs` endpoint with `provider:'gmaps'` + `category` + `city` + optional `radiusKm` + `maxResults`. Implemented in `scraping/gmapsScraper.ts`. Returns name/address/phone/website/category and creates leads with `source='gmaps_scrape'` + `email=NULL` + `email_status='needs_enrichment'` so the website-enrichment pipeline can find the real email.

  No email is ever invented: scraped Apollo rows whose address is masked are stored with `email=NULL` + `email_status='locked'` + `email_locked=true`; visible Apollo emails get `email_status='scraped'` + `email_locked=false` (still blocked from sending until enrichment verifies). LinkedIn rows always store `email=NULL` + `email_status='needs_enrichment'` — never a placeholder address. The send guard in `pipeline/sender.ts` uses an allowlist (`null` or `'verified'` only) — anything else is hard-blocked from sending until the enrichment module verifies a real email.
